...
var SpiritCardGrid = function () {
  function SpiritCardGrid() {
    _classCallCheck(this, SpiritCardGrid);
  }

  _createClass(SpiritCardGrid, null, [{
    key: 'init',
    value: function init() {
      // Get all the card grids in the dom and find any that are carousels
      var carouselGrids = Array.from(document.querySelectorAll('.spirit-card-carousel'));
      carouselGrids.forEach(function (c) {
        SpiritCardGrid.initializeEachCarousel(c);
      });
    }
  }, {
    key: 'initializeEachCarousel',
    value: function initializeEachCarousel(c) {
    ...
    key: 'addCarouselEventListeners',
    value: function addCarouselEventListeners(carousel) {
      var scrollingGrid = carousel.querySelector('.spirit-card-grid');
      var totalCards = carousel.querySelectorAll('.spirit-card').length;
      var paginationButtons = Array.from(carousel.querySelectorAll('.spirit-card-carousel__button'));
      var scrolling = void 0;
      var resizing = void 0;

      paginationButtons.forEach(function (b) {
        b.addEventListener('click', function (e) {
          var indicatorDots = Array.from(carousel.querySelectorAll('.spirit-card-carousel__indicator-dot'));
          var activeIndex = indicatorDots.findIndex(function (dot) {
            return dot.classList.contains('spirit-card-carousel__indicator-dot--active');
          }); // Get active index
          var selectedIndex = Math.min(indicatorDots.length, activeIndex + 1); // By default assume the next button was clicked
      ...
      scrollingGrid.addEventListener('scroll', function (e) {
        ...
        scrolling = window.requestAnimationFrame(function () {
          var scrollPosition = _this.scrollLeft;
          var totalScrollDistance = _this.scrollWidth - _this.clientWidth;

          var cardFloat = 1 / totalCards;
          var indicatorIndex = Math.min(Math.floor(scrollPosition / totalScrollDistance / cardFloat), totalCards - 1);
      ...
      window.addEventListener('resize', function (e) {
        ...
        resizing = window.requestAnimationFrame(function () {
          ...
          if (SpiritCardGrid.carouselIsActive(carousel)) {
            // This checks if the controls are visible, if they're not they'll have on offsetParent === null
    ...
    key: 'removeCarouselAriaAttribute',
    value: function removeCarouselAriaAttribute(carousel) {
      var cards = Array.from(carousel.querySelectorAll('.spirit-card'));
      cards.forEach(function (c) {
        return c.removeAttribute('aria-hidden');
      });
    }
  }, {
    key: 'carouselIsActive',
    value: function carouselIsActive(carousel) {
      var controls = carousel.querySelector('.spirit-card-carousel__controls');
    ...
    key: 'setActiveCardScrollPosition',
    value: function setActiveCardScrollPosition(carousel, activeIndex) {
      var cards = Array.from(carousel.querySelectorAll('.spirit-card'));
      var activeCard = cards[activeIndex];
      var scrollingGrid = carousel.querySelector('.spirit-card-grid');
      var cardWidth = activeCard.clientWidth;
      var viewportWidth = window.innerWidth;
      var scrollOffset = (viewportWidth - cardWidth) / 2;
      var scrollPosition = activeCard.offsetLeft - scrollOffset;

      scrollingGrid.scrollLeft = scrollPosition;
    }
  }, {
    key: 'setActiveCardAriaAttribute',
    value: function setActiveCardAriaAttribute(carousel, activeIndex) {
      var cards = Array.from(carousel.querySelectorAll('.spirit-card'));
      var activeCard = cards[activeIndex];
      cards.forEach(function (c) {
        return c.setAttribute('aria-hidden', 'true');
      });
      activeCard.setAttribute('aria-hidden', 'false');
    }
  }, {
    key: 'setActiveIndicator',
    value: function setActiveIndicator(carousel, activeIndex) {
      var dotActiveClass = 'spirit-card-carousel__indicator-dot--active';
      var indicatorDots = Array.from(carousel.querySelectorAll('.spirit-card-carousel__indicator-dot'));
      var cardCount = indicatorDots.length;
      var prevButton = carousel.querySelector('.spirit-card-carousel__button--prev');
      var nextButton = carousel.querySelector('.spirit-card-carousel__button--next');
      indicatorDots.forEach(function (d) {
        d.classList.remove(dotActiveClass);
      });
...
var spiritDialogs = function spiritDialogs() {
  if (typeof A11yDialog !== "function") {
    return;
  }

  var dialogs = Array.from(document.querySelectorAll('.spirit-dialog'));
  ...
  dialogs.forEach(function (item) {
    var dialog = new A11yDialog(item, defaults.container);
    var spiritDialog = item.querySelector('.spirit-dialog__content');

    var maybeDisableEscToClose = function maybeDisableEscToClose(e) {
    ...
    dialog.on('show', function (el) {
      ...
      function transitionEndCallback() {
        var focusableChildren = spiritDialog.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');
        var focused = spiritDialog.querySelector('[autofocus]') || focusableChildren[0];
...
var SpiritFormInputMasks = function () {
  function SpiritFormInputMasks() {
    _classCallCheck(this, SpiritFormInputMasks);
  }

  _createClass(SpiritFormInputMasks, null, [{
    key: 'enable',
  ...
  return SpiritFormInputMasks;
}();

SpiritFormInputMasks.enable();

var SpiritFormPasswordToggle = function () {
  function SpiritFormPasswordToggle() {
    _classCallCheck(this, SpiritFormPasswordToggle);
  }

  _createClass(SpiritFormPasswordToggle, null, [{
    key: 'enable',
    value: function enable() {
      var passwordToggleInputs = Array.from(document.querySelectorAll('.spirit-form__input--password-toggle'));
      passwordToggleInputs.forEach(function (i) {
        var input = i.querySelector('input');
        var trigger = i.querySelector('.spirit-form__input-password-toggle');
        var triggerIconHide = trigger.querySelector('svg[aria-label="Hide"]');
        var triggerIconShow = trigger.querySelector('svg[aria-label="Show"]');
  ...
  return SpiritFormPasswordToggle;
}();
 ...
 * @return {void}
 */
var spiritVerticalNavigation = function spiritVerticalNavigation() {
  var vertNavItems = Array.from(document.querySelectorAll('.spirit-vertical-nav'));
   ...
   * @return {void}
   */
  var initializeEachNav = function initializeEachNav(n) {
    var parentItems = Array.from(n.querySelectorAll('.spirit-vertical-nav__item-parent'));
    // const parentItemsCount = parentItems.length;

    var menuSubOpen = function menuSubOpen(closedItem) {
     ...
     */
    var menuSubClose = function menuSubClose(openItem) {
     ...
     * @param {object} e event
     */
    var listnerSubmenuClick = function listnerSubmenuClick(e) {
      ...
      if (target.getAttribute('aria-haspopup')) {
        ...
        var allOpenMenuTriggers = Array.from(n.querySelectorAll('.child-has-focus > a.submenu-is-open:not(.spirit-vertical-nav__link--expanded)'));
        var allOpenMenuTriggersCount = allOpenMenuTriggers.length;

        if (allOpenMenuTriggersCount > 0) {

          // Make sure only 1 menu item can be opened at a time
          allOpenMenuTriggers.forEach(function (t) {
     ...
     */
    var listnerSubmenuFocus = function listnerSubmenuFocus(evt) {
      ...
      var parentMenu = target.parentNode;
      var allOpenMenuTriggers = Array.from(n.querySelectorAll('.child-has-focus > a.submenu-is-open'));
      var allOpenMenuTriggersCount = allOpenMenuTriggers.length;
    ...
    parentItems.forEach(function (p) {
      var trigger = p.querySelector('a');
      var subNav = p.querySelector('.spirit-vertical-nav__subnav');
   ...
   */
  vertNavItems.forEach(function (n) {
    initializeEachNav(n);
  });
};
